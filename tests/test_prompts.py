...
def prompts_dir() -> Path:
    """Return the project root directory (where registry.yaml lives)."""
...
def test_all_yaml_files_valid(prompts_dir: Path, registry_data: Dict[str, Any]):
    """Test if registry.yaml and all referenced YAML files are valid."""
    yaml_files: List[Path] = [prompts_dir / "registry.yaml"]

    for agent_data in registry_data.get("agents", {}).values():
        prompt_path = prompts_dir / agent_data["path"]
        yaml_files.append(prompt_path)
        yaml_files.append(prompt_path.parent / "prompt.tests.yaml")
...
@pytest.fixture(scope="session")
def all_prompts(
    prompts_dir: Path, registry_data: Dict[str, Any]
) -> List[Tuple[str, Path]]:
...
def test_prompt_structure(all_prompts):
    ...
    for prompt_id, prompt_path in all_prompts:
        ...
        missing_fields = [
            field for field in required_fields if field not in prompt_data
        ]
...
@pytest.fixture(scope="session")
def all_test_cases(
    prompts_with_tests: List[Tuple[str, Path, Path]],
...
def test_test_cases_structure(prompts_with_tests):
    ...
    for prompt_id, prompt_path, test_path in prompts_with_tests:
        ...
        assert isinstance(test_data["cases"], list), (
            f"'cases' must be a list in {prompt_id}"
        )
        ...
        for i, case in enumerate(test_data["cases"]):
            ...
            if missing:
                pytest.fail(
                    f"Prompt {prompt_id}, case '{case_name}' is missing: {missing}"
                )
...
def test_prompt_rendering_with_test_cases(all_test_cases):
    ...
    for case_id, prompt_data, test_case in all_test_cases:
        ...
        missing_texts = [
            text for text in test_case["expect_contains"] if text not in rendered
        ]
        if missing_texts:
            pytest.fail(
